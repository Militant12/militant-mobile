...
class CallService {
  ...
  /// Répondre à un appel
  Future<void> answerCall(String callId, String offerSdp, String callType) async {
    try {
    ...
    _pollTimer = Timer.periodic(Duration(seconds: 2), (timer) async {
      ...
      try {
        ...
        if (call['status'] == 'ended') {
          ...
          await cleanup();
          return;
        } else if (call['status'] == 'active' && call['answer_sdp'] != null) {
          // Définir la réponse distante si on est l'appelant
  ...
  void _startConnectivityMonitoring() {
    ...
    _connectivitySubscription = Connectivity().onConnectivityChanged.listen((result) async {
      ...
      if (hasConnection && !_isRestartingIce) {
        print('Network change detected, restarting ICE...');
